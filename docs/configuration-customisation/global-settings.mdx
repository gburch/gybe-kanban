...
You can customise how Git branches and merge commits are created:

- **Branch prefix** – change the `vk/` prefix that is added to task branches, or disable it by leaving the field blank.
- **Merge commit suffix** – control or remove the `(vibe-kanban 37f9fa85)` suffix that is appended to merge commit titles. Use `{short_id}` or `{task_id}` placeholders for dynamic values.
...
Link additional Git repositories to a project from the project detail view (Settings → Repositories). Configure the path and optional root to scope the files that agents can access.

<Card title="Add repositories to a project" icon="git-branch" href="/core-features/creating-projects#managing-multiple-repositories">
  Step-by-step instructions, examples, and limitations for multi-repo setups
</Card>
