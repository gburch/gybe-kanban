...
const CreatePrDialog = NiceModal.create(() => {
      ...
      if (data.projectId) {
        ...
        Promise.all([
          ...
          .then(([projectBranches, projectRemotes]) => {
            ...
            const remoteFromBranch = getRemoteFromBranchName(data.attempt.branch);
  ...
  useEffect(() => {
    ...
    if (remoteFromBranch && remotes.some((remote) => remote.name === remoteFromBranch)) {
      if (!selectedRemote) {
      ...
      <Dialog open={modal.visible} onOpenChange={() => handleCancelCreatePR()}>
        <DialogContent className="sm:max-w-[525px]">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="pr-remote">Target Remote</Label>
              <Select
