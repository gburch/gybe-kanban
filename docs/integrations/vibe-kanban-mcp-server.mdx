...
<Tip>
When a coding agent is running inside a Vibe Kanban task attempt, the container exports the environment variable `VIBE_PARENT_TASK_ATTEMPT_ID`. Pass this value to the MCP `create_task` tool via the `parent_task_attempt` argument so newly created tasks are linked as subtasks of the active attempt. If you only know the parent task ID, provide it via the `parent_task_id` argument and the server will resolve the active attempt automatically.
</Tip>
